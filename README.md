...
Unity-разработчик (C#) | Выпустил игры на Яндекс Играх | Архитектура, Editor Scripting, WebGL

Специалист разработки на C# и Unity. Питаю глубокий интерес к технологиям и ищу коллектив для продвижения в профессии.

Что уже делаю:
Механики и геймплей: Полный игровой цикл (экономика, NPC с конечным автоматом, очередь, система контрактов, парковочные слоты), система бонусов с зарядами, система кнопок-переключателей с параметрами.
UI и метаигра: Диалоги на ScriptableObject (эффект печатной машинки, блокировка кликов/камеры), привязка UI-кнопок к 3D-объектам (Screen/World Space), менеджер всплывающего текста с пулом объектов, прогресс-бары.
ООП и жизненный цикл Unity: Инкапсуляция состояния, разделение логики и представления (GridModel без зависимостей от Unity, GridCellView — только отрисовка), событийная коммуникация (event Action) с корректной отпиской в OnDestroy, контроль жизненного цикла менеджеров (singleton с корректной отпиской).
Сцены, префабы, async: Editor Scripting (SceneBuilder, генераторы префабов, тем, shape library, автонастройка проекта), работа с корутинами и асинхронной логикой, пулы объектов.
Git и командная работа: Публичные репозитории с README и описанием архитектуры, опыт прохождения код-ревью, разбор чужого кода.
AI-инструменты: Ежедневно использую Claude и ChatGPT для генерации игровых систем, рефакторинга и написания Editor-инструментов — это плюс по вакансии.

Мои ключевые качества:
Стремление к обучению: Оперативно осваиваю необходимые для проекта инструменты и технологии (готов быстро подтянуть Zenject/VContainer, UniTask, профилирование, CI/CD).
